...
namespace DataTools.Common
{
    ...
    public class EPPlusCommon
    {
        ...
        public static byte[] ContinueWriteExcelByFileReturnByte(string filePath, Dictionary<string, object> dataDic, int sheetIndex = 0, string sheetNameName = null)
        {
            FileInfo newFile = new FileInfo(filePath);
            using (var stream = new MemoryStream())
            {
        ...
        public static IEnumerable<T> GetListByExcel<T>(FileInfo existingFile) where T : new()
        {
